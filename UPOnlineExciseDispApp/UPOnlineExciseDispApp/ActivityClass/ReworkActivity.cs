...
{
    [Activity(Label = "UP Online Excise Rework App", WindowSoftInputMode = SoftInput.StateHidden, ConfigurationChanges = ConfigChanges.ScreenSize | ConfigChanges.Orientation, ScreenOrientation = ScreenOrientation.Portrait)]
    public class ReworkActivity : AppCompatActivity
    {
        ...
        private void txtScanBottelBarcode_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        lblMsg.Text = "";
                        if (txtScanBottelBarcode.Text.Length != 34 && txtScanBottelBarcode.Text.Length != 44)
                        {
                        ...
                        if (!lblScanCount.Text.Equals("0/0"))
                        {
                            if (lblScanCount.Text.Trim().Split('/')[0].Equals(lblPackSize.Text.Trim()))
                            {
                                if (txtScanBottelBarcode.Text.Length != 34)
                                {
                        ...
                        if (txtScanBottelBarcode.Text.Trim().Length == 34)
                        {
        ...
        private void txtScanBarcode_KeyPress(object sender, View.KeyEventArgs e)
        {
            try
            {
                if (e.Event.Action == KeyEventActions.Down)
                {
                    ...
                    {
                        lblMsg.Text = "";
                        if (txtScanBarcode.Text.Length != 44 && txtScanBarcode.Text.Length != 34)
                        {
                            lblMsg.Text = "Invalid Barcode!!!";
                            lblMsg.SetTextColor(Android.Graphics.Color.Red);
                            txtScanBarcode.Text = "";
                        ...
                        if (txtScanBarcode.Text.Trim().Length == 34)
                        {
        ...
        private string[] SendDataToServer(string Type, string lblPlant, string lblLine, string lblETIN, string lblBatchNo, string lblPackSize, string lblStregth, string lblVolume, string lblRefNo, string lblPlanId)
        {
            ...
            try
            {
                ...
                sReturn = oNetwork.GetSetTcpNewApp(_MESSAGE);
                //WriteLogMess(Type + ": : SendDataToServer", sReturn);
                string[] _RESPONSE = sReturn.Split('~');
                return _RESPONSE;
            }
            catch (Exception ex)
            {
            //    WriteLogMess("Exception SendDataToServer", ex.Message);
            //    WriteLogMess("Exception SendDataToServer", sReturn);


                throw ex;
            }
        }
        void WriteLogMess(string method,string msg)
        {
            string folderPath = Path.Combine(clsGlobal.FilePath, clsGlobal.FileFolder);
            string filename = Path.Combine(folderPath, clsGlobal.DeviceLog);

            using (var streamWriter = new StreamWriter(filename, true))
            {
                streamWriter.WriteLine(""+ method + " :: " + msg);
            }
        }
