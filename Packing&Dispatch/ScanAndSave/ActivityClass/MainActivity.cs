...
{
    [Activity(Label = "UP Offline App", MainLauncher = true, WindowSoftInputMode = SoftInput.StateHidden, ConfigurationChanges = ConfigChanges.ScreenSize | ConfigChanges.Orientation, ScreenOrientation = ScreenOrientation.Portrait)]
            ...
            try
            {
                if (e.Event.Action == KeyEventActions.Down)
                {
                    if (e.KeyCode == Keycode.Enter)
                    {
                        ...
                        if (string.IsNullOrEmpty(txtScanCase.Text.Trim()))
                        {
                            StartPlayingSound();
                            ShowMessageBox("Scan case", this);
                            //Toast.MakeText(this, "Scan case", ToastLength.Long).Show();
                            txtScanCase.RequestFocus();
                            return;
                        }
                        if (txtScanCase.Text.Contains(">"))
                        {
                            StartPlayingSound();
                            ShowMessageBox("This is invalid case barcode because case having > invalid character", this);
                            //Toast.MakeText(this, "This is invalid case barcode because case having > invalid character", ToastLength.Long).Show();
                            txtScanCase.RequestFocus();
                            return;
                        }
                        if (txtScanCase.Text.Length == 35)
                        {
                        ...
                        {
                            StartPlayingSound();
                            ShowMessageBox("Wrong Barcode scanned", this);
