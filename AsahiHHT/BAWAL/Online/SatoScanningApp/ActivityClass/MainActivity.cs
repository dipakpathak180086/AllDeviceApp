...
namespace AISScanningApp
{
    ...
    public class MainActivity : AppCompatActivity
    {
        ...
        Vibrator vibrator;
        Button btnVerifiedPrinting, btnMisVerifiedPrinting;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                btnVerifiedPrinting = FindViewById<Button>(Resource.Id.btnVerifiedPrinting);
        ...
        private void BtnMisVerifiedPrinting_Click(object sender, EventArgs e)
        {
            try
            {
                clsGlobal.Process_Header_Text = "Manual Printing Verification";
                OpenActivity(typeof(ManualVerifedPrintingActivity));
            }
            catch (Exception ex)
            {
                clsGLB.ShowMessage(ex.Message, this, MessageTitle.ERROR);
        ...
        private void btnVerifiedPrinting_Click(object sender, EventArgs e)
        {
            try
            {
                clsGlobal.Process_Header_Text = "Printing Verification";
                OpenActivity(typeof(VerifedPrintingActivity));
            }
            catch (Exception ex)
            {
                clsGLB.ShowMessage(ex.Message, this, MessageTitle.ERROR);
        ...
        private void DisableMenus()
        {
            try
            {
                ...
                btnVerifiedPrinting.Enabled = false;
                btnMisVerifiedPrinting.Enabled = false;
                ...
                switch (_RESPONSE[0])
                {
                    case "VALID":
                        ...
                        for (int i = 0; i < sArr.Length; i++)
                        {
                            ...
                            {
                                case "201":
                                    btnVerifiedPrinting.Enabled = true;
                                    break;
                                case "202":
                                    btnMisVerifiedPrinting.Enabled = true;
                                    break;
                                default:
                                    break;
                            }
