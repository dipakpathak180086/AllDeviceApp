    ...
    class ModNet : Activity
    {
        ...
        public static bool FetchBatteryDeliveryData(string strDeliveryNo)
        {
            try
            {
                ...
                ModInit.Gstrarr = ModInit.GstrResult.Split("~");
                if (ModInit.Gstrarr[0] ==  strDeliveryNo.PadLeft(10,'0'))
                    return true;
                else
                    return false;
            }
        ...
        public static bool FetchBatteryDeliverySTOData(string strDeliveryNo)
        {
            try
            {
                SendDataToServer("SAP_BAT_FETCH_DELIVERY_DATA_STO^" + strDeliveryNo);
                //Application.DoEvents();
                ModInit.GstrResult = ReceiveDataFromServer();
        ...
        public static bool ValidateBatteryDataSTO(string strDeliveryNo, string strSKU, string strBatteryBatteryBarcode)
        {
            try
            {
                SendDataToServer("SAP_VALIDATE_BAT_DATA_STO^" + strDeliveryNo + "^" + strSKU + "^" + strBatteryBatteryBarcode);
                //Application.DoEvents();
                ModInit.GstrResult = ReceiveDataFromServer();
        ...
        public static bool GetBatteryTypeDataSTO(string strBatteryBatteryBarcode, ref string strBatType)
        {
            try
            {
                SendDataToServer("SAP_GET_BAT_TYPE_DATA_STO^" + strBatteryBatteryBarcode);
                //Application.DoEvents();
                ModInit.GstrResult = ReceiveDataFromServer();
                ModInit.Gstrarr = ModInit.GstrResult.Split("~");
                if (ModInit.Gstrarr[0] == "OK")
        ...
        public static bool BatteryScanningDataSaveSTO(string strDeliveryNo, string strSKU, string strBatteryBarcode,string strPlant)
        {
            try
            {
                SendDataToServer("SAP_BAT_SAVE_DATA_STO^" + strDeliveryNo + "^" + strSKU + "^" + strBatteryBarcode + "^" + strPlant);
                //Application.DoEvents();
                ModInit.GstrResult = ReceiveDataFromServer();
                ModInit.Gstrarr = ModInit.GstrResult.Split("~");
