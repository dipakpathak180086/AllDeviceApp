...
namespace SatoScanningApp
{
    ...
    public class DirectDispatchActivity : AppCompatActivity
    {
        ...
        {
            DirectDispatchActivity scanFragment;
            public UIHand(DirectDispatchActivity _scanFragment)
            {
        ...
        private class InitTask : AsyncTask<Java.Lang.Void, Java.Lang.Void, string[]>
        {
            ...
            DirectDispatchActivity mainActivity;
            ProgressDialog proDialg = null;

            public InitTask(DirectDispatchActivity _mainActivity)
            {
        ...
        public void GetDocNumber()
        {
            try
            {
                string _MESSAGE = "DISPATCH~BIND_DOC~" + clsGlobal.mPlant + "~" + "~" + "~" + "~" + "~" + "~" + "}";
        ...
        public void GetToPlant()
        {
            try
            {
                string _MESSAGE = "DISPATCH~BIND_TO_PLANT~" + clsGlobal.mPlant + "~" + "~" + cmbDocNo.SelectedItem.ToString() + "~" + "~" + "~" + "~" + "}";
                string[] _RESPONSE = oNetwork.GetSetTcpNewApp(_MESSAGE).Split('~');
        ...
        private void GetDocDetails(string sRequestNo)
        {
            try
            {
                ...
                sRequest = oNetwork.GetSetTcpNewApp("DISPATCH~BIND_VIEW~" + clsGlobal.mPlant + "~" + "~" + sRequestNo + "~" + "~" + "~" + "~" + "~" + "}");
        ...
        private string[] CompleteDispatchDataToServer()
        {
            try
            {
                string _MESSAGE = "DISPATCH~COMPLETE~" + clsGlobal.mPlant + "~" + txtToPlant.Text.Trim() + "~" + cmbDocNo.SelectedItem.ToString().Trim() + "~" + editAssetBarcode.Text.Trim() + "~" + clsGlobal.UserId + "}";
                string[] _RESPONSE = oNetwork.GetSetTcpNewApp(_MESSAGE).Split('~');
                return _RESPONSE;
            }
            catch (Exception ex)
        ...
        private string[] ScanDispatchDataToServer()
        {
            try
            {
                ...
                for (int i = 0; i < _ListItem.Count; i++)
                {
                    ...
                    string _MESSAGE = "DISPATCH~SAVE~" + clsGlobal.mPlant + "~" + txtToPlant.Text + "~" + cmbDocNo.SelectedItem.ToString().Trim() + "~" + _ListItem[i].ScannedRFIDTag.ToString().Trim() + "~" + clsGlobal.UserId + "}";
