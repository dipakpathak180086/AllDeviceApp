...
namespace SatoScanningApp
{
    ...
    public class TagUnMappingActivity : AppCompatActivity
    {
        SoundPool soundPool;
        int soundPoolId;
        ...
        TextView txtAssetCode, txtMsg, txtAssetDesc, txtPlant;
        ...
        {
            TagUnMappingActivity scanFragment;
            public UIHand(TagUnMappingActivity _scanFragment)
            {
        ...
        private class InitTask : AsyncTask<Java.Lang.Void, Java.Lang.Void, string[]>
        {
            ...
            TagUnMappingActivity mainActivity;
            ProgressDialog proDialg = null;

            public InitTask(TagUnMappingActivity _mainActivity)
            {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            try
            {
                ...
                SetContentView(Resource.Layout.layout_TagUnMapping);
                if (uhfAPI == null)
                {
                    try
                    {
        ...
        public void GetDataDetails()
        {
            try
            {
                string _MESSAGE = "TAGUNMAPPING~BIND_DTL~" + txtPlant.Text + "~" + txtAssetCode.Text.Trim() + "~" + txtAssetDesc.Text.Trim() + "~" + editAssetBarcode.Text.Trim() + "~" + clsGlobal.UserId + "}";
                        ...
                        if (!_RESPONSE[1].Equals("Y"))
                        {
                            ...
                            if (sArr.Length > 0)
                            {
                                ...
                                if (sArr2.Length > 1)
                                {
                                    ...
                                    txtAssetCode.Text = sArr2[1].ToString();
                                    txtAssetDesc.Text = sArr2[2].ToString();
        ...
        async Task<string> ScanSaveDataAsync()
        {
            ...
            {
                txtMsg.Text = "";
                string[] _RESPONSE = await Task.Run(() => ScanSaveDataToServer());
                ...
                switch (_RESPONSE[0])
                {
                    case "VALID":
                        ...
                        {
                            Sound();
                            txtMsg.Text = $"Tag {editAssetBarcode.Text.Trim()} Unmapped Successfully";
                            editAssetBarcode.Enabled = true;
                            editAssetBarcode.Text = "";
                            editAssetBarcode.RequestFocus();
        ...
        private string[] ScanSaveDataToServer()
        {
            try
            {
                string _MESSAGE = "TAGUNMAPPING~UNMAPPED~" + txtPlant.Text + "~" + txtAssetCode.Text.Trim() + "~" + txtAssetDesc.Text.Trim() + "~" + editAssetBarcode.Text.Trim() + "~" + clsGlobal.UserId + "}";
                string[] _RESPONSE = oNetwork.GetSetTcpNewApp(_MESSAGE).Split('~');
                return _RESPONSE;
            }
            catch (Exception ex)
